...
KmerDistribution BWTAlgorithms::totalKmerCounts(size_t kmerSize, const BWT* pBWT)
{
    KmerDistribution distribution;
    size_t n = pBWT->getNumStrings();
    for(size_t idx = 0; idx < n; ++idx)
    {
        std::string current_sequence = extractString(pBWT, idx);
        //std::cout << current_sequence << std::endl;
        size_t num_kmers = current_sequence.length() - kmerSize + 1;
        for(size_t j = 0; j < num_kmers; ++j)
        {
            std::string kmer_sequence = current_sequence.substr(j, kmerSize);
            //std::cout << kmer_sequence << std::endl;
            int count = countSequenceOccurrences(kmer_sequence, pBWT);
            distribution.add(count);
        }
    }
    return distribution;
