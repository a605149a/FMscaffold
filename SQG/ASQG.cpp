...
// ScaEdgeRecord
//
ScaEdgeRecord::ScaEdgeRecord(const std::string& recordLine)
{
    parse(recordLine);
}
//
void ScaEdgeRecord::write(std::ostream& out)
{
    StringVector fields;
    fields.push_back(EDGE_TAG);
    std::stringstream ss;
...
void ScaEdgeRecord::parse(const std::string& record)
{
    if(record.size() < 2)
    {
        std::cerr << "Error: Record is not valid\n";
    ...
    std::stringstream ssparser1(tokens[1]);
    ssparser1 >> m_StartID;
    std::stringstream ssparser2(tokens[2]);
    ssparser2 >> m_EndID;
    std::stringstream ssparser3(tokens[3]);
    ssparser3 >> m_EC_comp;
    std::stringstream ssparser4(tokens[4]);
    ssparser4 >> m_ED_dir;
    std::stringstream ssparser5(tokens[5]);
    ssparser5 >> m_npair;
    std::stringstream ssparser6(tokens[6]);
    ssparser6 >> m_C1Pos;
    std::stringstream ssparser7(tokens[7]);
    ssparser7 >> m_C2Pos;
}
//
//
