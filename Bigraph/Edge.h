...
{
    public:
        Edge(Vertex* end, EdgeDir dir, EdgeComp comp, SeqCoord m , GraphColor c= GC_WHITE) : 
                 m_pEnd(end), m_pTwin(NULL), m_matchCoord(m), m_color(c), isTrusted(false)
        {
            m_edgeData.setDir(dir);
            m_edgeData.setComp(comp);
        }
        Edge(Vertex* end, EdgeDir dir, EdgeComp comp, SeqCoord m , size_t npair, GraphColor c= GC_WHITE) : 
                 m_pEnd(end), m_pTwin(NULL), m_matchCoord(m), m_color(c), m_npair(npair), isTrusted(false)
        {
            m_edgeData.setDir(dir);
            m_edgeData.setComp(comp);
        }
        //----- scaffold edge ----------
        // for make a scaffold graph
        Edge(Vertex* end, EdgeDir dir, EdgeComp comp,  size_t npair, 
             size_t c1_pos_median, size_t c2_pos_median,
             std::vector<SeqItem> r1_reads, std::vector<SeqItem> r2_reads, GraphColor c= GC_WHITE) : 
                 m_pEnd(end), m_pTwin(NULL), m_color(c), m_npair(npair),
                 m_c1_pos_median(c1_pos_median), m_c2_pos_median(c2_pos_median),
                 m_r1_reads(r1_reads), m_r2_reads(r2_reads), isTrusted(false)
        {
            m_edgeData.setDir(dir);
            m_edgeData.setComp(comp);
            m_nlength = 0;
        }
        // for load scaffold asqg
        Edge(Vertex* end, EdgeDir dir, EdgeComp comp,  size_t npair,
            size_t c1_pos_median, size_t c2_pos_median, GraphColor c= GC_WHITE) : 
                 m_pEnd(end), m_pTwin(NULL), m_color(c), m_npair(npair),
                 m_c1_pos_median(c1_pos_median), m_c2_pos_median(c2_pos_median),isTrusted(false)
        {
            m_edgeData.setDir(dir);
            m_edgeData.setComp(comp);
            m_nlength = 0;
        ...
        // scaffold
        void joinScaffold(const Edge* pEdge);        
        void extendScaffold(const Edge* pEdge);
        ...
        // scaffold        
        void setNpair(size_t npair) { m_npair = npair; }
        size_t getNpair() const { return m_npair;}
        void setNLength(size_t nlength) { m_nlength = nlength; }
        size_t getNLength() const { return m_nlength; }
        
        size_t getC1PosMedian() const { return m_c1_pos_median;}
        size_t getC2PosMedian() const { return m_c2_pos_median;}
        
        
        std::vector<SeqItem> getR1Reads() const { return m_r1_reads;}
        std::vector<SeqItem> getR2Reads() const { return m_r2_reads;}
        ...
        // Self-memory management disabled by YTH due to not thread-safe
		...
		{
			std::cout << m_pEnd->getID() << "," << this->getDir()  << "," << this->getComp() << ","  << m_matchCoord << "," << (int)m_color	;
		}
    ...
    protected:
        ...
        size_t m_c1_pos_median;
        size_t m_c2_pos_median;
        std::vector<SeqItem> m_r1_reads;
        std::vector<SeqItem> m_r2_reads;

    public:
        bool isTrusted;
};
class scaffoldEdge:public Edge
{
};
#endif
