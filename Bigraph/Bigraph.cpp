...
// scaffold
void Bigraph::mergeScaffold(size_t insertSize, Vertex* pV1, Edge* pEdge, EdgeDir dir)
{
	Vertex* pV2 = pEdge->getEnd();
	// Merge the data
	pV1->mergeScaffold(insertSize, pEdge, dir);
	...
	{
        Edge* pTransEdge = *iter;
		// std::cout << pTransEdge->getStart()->getID() << " to " << pTransEdge->getEnd()->getID() << std::endl;
		// Remove the edge from V2, this does not destroy the edge
		pV2->removeEdge(pTransEdge);
	...
	// It is guarenteed to not be connected
	removeIslandVertex(pV2);
    // writeScaffoldDot("simplify.dot", 0); 
}
...
//    Simplify the graph by compacting singular edges
void Bigraph::simplifyScaffold(size_t insertSize)
{
	assert(!hasContainment());
	size_t mergeCount = 0 ;
	...
	while(iter != m_vertices.end())
	{
		mergeCount += simplifyScaffold(insertSize, iter->second, ED_SENSE);
		mergeCount += simplifyScaffold(insertSize, iter->second, ED_ANTISENSE);
		++iter;
	}
...
//merge unipaths from pV to pW in EdgeDir
size_t Bigraph::simplifyScaffold(size_t insertSize, Vertex* pV, EdgeDir dir)
{
	size_t mergeCount = 0 ;
	// Get the edges for this direction
	EdgePtrVec edges = pV->getEdges(dir);
	
    // std::cout << "pV ID: " << pV->getID() << std::endl;
	...
	{  
        ...
        size_t edgeSize;        
        edgeSize = pW->countEdges(pTwin->getDir());
        
        // std::cout << "pW ID: " << pW->getID() << std::endl;
        // std::cout << "pW edge size: " << edgeSize << std::endl;
        if(edgeSize == 1)
		{			
            //merge pW seq into pV and move its transitive edges to pV
			mergeScaffold(insertSize, pV, pSingle, dir);
			mergeCount++ ;
			//remove self edges produced by V->W->V=V<->V
            // std::cout << "new pV ID: " << pV->getID() << std::endl;
...
// Write the scaffold graph
void Bigraph::writeScaffoldDot(const std::string& filename, int dotFlags) const
{
	std::ofstream out(filename.c_str());
	...
	for(; iter != m_vertices.end(); ++iter)
	{
        ...
        // size_t StartPos = 0;
        // size_t EndPos = 0;
        // while(getline(ss2,sub_str,'_'))
        // {
            // if(count == 1)
                // StartPos = atoi(sub_str.c_str());
            // if(count == 2)
                // EndPos = atoi(sub_str.c_str());
            // count += 1;
        // }  
		...
		std::string len = ss.str();
		std::string label = (dotFlags & DF_NOID) ? "" : id+":"+len;
        // std::string color = (StartPos <= EndPos) ? "black" : "red";
		...
		out << " style=\"filled\" fillcolor=\"" << getColorString(iter->second->getColor()) << "\" ";
		out << "];\n";
		iter->second->writeScaffoldEdges(out, dotFlags);
	}
	out << "}\n";
	out.close();
}
void Bigraph::writeScaffoldASQG(const std::string& filename) const
{
	std::ostream* pWriter = createWriter(filename);
	...
	for(iter = m_vertices.begin(); iter != m_vertices.end(); ++iter)
	{
		...
		for(EdgePtrVecIter edgeIter = edges.begin(); edgeIter != edges.end(); ++edgeIter)
		{
			// We write one record for every bidirectional edge so only write edges
			// that are in canonical form (where id1 < id2)
            if((*edgeIter)->getColor() != GC_BLACK)
            {
                
                VertexID StartID = (*edgeIter)->getStartID();
                VertexID EndID = (*edgeIter)->getEndID();
                
                // EdgeComp EC_comp = (*edgeIter)->getComp();
                std::string EC_comp;
                if((*edgeIter)->getComp() == EC_SAME)
                    EC_comp = "EC_SAME";
                else    
                    EC_comp = "EC_REVERSE";
                    
                std::string ED_dir;    
                if((*edgeIter)->getDir() == ED_SENSE)
                    ED_dir = "ED_SENSE";
                else    
                    ED_dir = "ED_ANTISENSE";  
                    
                size_t npair = (*edgeIter)->getNpair();
                
                size_t C1Pos = (*edgeIter)->getC1PosMedian();
                size_t C2Pos = (*edgeIter)->getC2PosMedian();
                ASQG::ScaEdgeRecord scaEdgeRecord(StartID, EndID, EC_comp, ED_dir, npair, C1Pos, C2Pos);
                        scaEdgeRecord.write(*pWriter);
                (*edgeIter)->setColor(GC_BLACK);   
                (*edgeIter)->getTwin()->setColor(GC_BLACK);          
            }                    
		}
	}    
	delete pWriter;
