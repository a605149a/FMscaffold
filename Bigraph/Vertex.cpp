...
// scaffold
void Vertex::mergeScaffold(size_t insertSize, Edge* pEdge, EdgeDir dir)
{            
    std::string scaffoldId = "";
    ...
    // std::cout << "C1 ID: " << pEdge->getStart()->getID() << ", C1 Len: " << pEdge->getStart()->getSeqLen() << std::endl;
    // std::cout << "C2 ID: " << pEdge->getEnd()->getID() << ", C2 Len: " << pEdge->getEnd()->getSeqLen() << std::endl;
    // std::cout << "C1 pos: " << pEdge->getC1PosMedian() << ", C2 pos: " << pEdge->getC2PosMedian() << std::endl;
    // size_t N_Length = 200;
    size_t N_Length = insertSize - pEdge->getNLength();
    // std::cout << N_Length << std::endl;
    std::string oVid = "";
    std::string oVseq = "";
    std::string oWid = "";
    std::string oWseq = "";  
    if(dir == ED_SENSE)
    {
        ...
        else
        {
            ...
            oVseq = pVseq;
            oWid = "rev_" + pW->getID();
            oWseq = reverseComplement(pWseq);
        }
        scaffoldId = oWid + "+" + oVid;
        scaffoldSeq.append(oWseq);
        for(size_t i = 0; i < N_Length; i++)
            scaffoldSeq.append("N");
        scaffoldSeq.append(oVseq);
    }
    else
    {
        if(pEdge->getComp() == EC_SAME)
    ...
    setID(scaffoldId);
    // std::cout << scaffoldId << std::endl;
    setSeq(scaffoldSeq);
    // std::cout << getSeqLen() << std::endl;
    // std::cout << scaffoldSeq << std::endl;
    setScaSeq(scaffoldSeq);
    setScaffold(true);
...
void Vertex::sortAdjListByMatchLen()
{
    EdgeMatchLenComp comp;
    std::sort(m_edges.begin(), m_edges.end(), comp);
}
...
bool Vertex::markDuplicateEdges(EdgeDir dir, GraphColor dupColor)
{
    for(EdgePtrVecIter iter = m_edges.begin(); iter != m_edges.end(); ++iter)
    {
        ...
        if(pEdge->getDir() == dir)
        {
            ...
            {
                // std::cerr << getID() << " has a duplicate edge to " << pEdge->getEndID() << " with overlap " << pEdge->getMatchLength() << "\n";
...
//get the edges EC_SAME and EC_REVERSE
EdgePtrVec Vertex::getEdgesBoth(EdgeDir dir) const
{
    EdgePtrVecConstIter iter = m_edges.begin();
    EdgePtrVec outEdges;
    for(; iter != m_edges.end(); ++iter)
    {
        if((*iter)->getComp() == EC_SAME)
        {
            if((*iter)->getDir() == dir)
            outEdges.push_back(*iter);
        }
...
//get the edges EC_SAME
EdgePtrVec Vertex::getEdgesSame(EdgeDir dir) const
{
    EdgePtrVecConstIter iter = m_edges.begin();
    EdgePtrVec outEdges;
    for(; iter != m_edges.end(); ++iter)
...
//get the edges EC_REVERSE
EdgePtrVec Vertex::getEdgesRev() const
{
    EdgePtrVecConstIter iter = m_edges.begin();
    EdgePtrVec outEdges;
    for(; iter != m_edges.end(); ++iter)
...
EdgePtrVec Vertex::getEdgesWithout(Vertex* pV) const
{
    EdgePtrVecConstIter iter = m_edges.begin();
    EdgePtrVec outEdges;
    for(; iter != m_edges.end(); ++iter)
...
EdgePtrVec Vertex::getEdgesWith(EdgeDir dir, Vertex* pV) const
{
    EdgePtrVecConstIter iter = m_edges.begin();
    EdgePtrVec outEdges;
    for(; iter != m_edges.end(); ++iter)
    {
        if((*iter)->getDir() == dir && (*iter)->getEnd() == pV)
            outEdges.push_back(*iter);  
    }
    return outEdges;
}
EdgePtrVec Vertex::getScaffoldEdges(EdgeDir dir, EdgeComp comp) const
{
    EdgePtrVecConstIter iter = m_edges.begin();
    EdgePtrVec outEdges;
    for(; iter != m_edges.end(); ++iter)
    {
        if((*iter)->getDir() == dir && (*iter)->getComp() == comp)
            outEdges.push_back(*iter);  
    }
    return outEdges;
}
...
// scaffold
size_t Vertex::countEdges(EdgeDir dir, EdgeComp comp)
{
    EdgePtrVec ev = getScaffoldEdges(dir, comp);
    return ev.size();
}
size_t Vertex::countScaffoldEdges(Vertex* pV)
{
    EdgePtrVec ev = getEdgesWithout(pV);
    return ev.size();
}
...
//--- Output Scaffold edge is npair count
void Vertex::writeScaffoldEdges(std::ostream& out, int dotFlags) const
{
    EdgePtrVecConstIter iter = m_edges.begin();
    for(; iter != m_edges.end(); ++iter)
    {
        ...
        {
            // if((*iter)->getDir() == ED_SENSE)
            // {
                out << "\"" << (*iter)->getStartID() << "\" -> \"" << (*iter)->getEndID();
                // std::string color = ((*iter)->getComp() == EC_SAME) ? "black" : "red";
                std::string color = ((*iter)->getDir() == ED_SENSE) ? "black" : "red";
                out << "\" [color=\"" << color << "\" ";
                
                if( (*iter)->getComp() == EC_SAME )            
                    out << "label=\"" << (*iter)->getNpair() << ", SAME" << "\""; // << "dir=\"none\"";
                else    
                    out << "label=\"" << (*iter)->getNpair() << ", REV"  << "\""; // << "dir=\"none\"";     
                out << "];";
            // }
        }
        out << "\n";
...
// Compare string edge points by match length
bool EdgeMatchLenComp::operator()(const Edge* pA, const Edge* pB)
{
    return pA->getMatchLength() < pB->getMatchLength();
}
