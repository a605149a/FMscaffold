...
#include <sys/stat.h>
bool SGFastaVisitor2::visit(StringGraph* /*pGraph*/, Vertex* pVertex)
{
	size_t seqLen = pVertex->getSeqLen() ;
	m_fileHandle << ">" << pVertex->getID() << " len:" << seqLen << "\n";
    if(pVertex->isScaffold())
        m_fileHandle << pVertex->getScaSeq() << "\n";
    else
        m_fileHandle << pVertex->getStr() << "\n";
        
    // std::cout << pVertex->getID() << std::endl; 
    // std::cout << pVertex->getStr() << std::endl;      
	return false;
}

void SGScaffoldVisitor::previsit(StringGraph* pGraph)
{
    pGraph->setColors(GC_WHITE);
}
bool SGScaffoldVisitor::visit(StringGraph* /*pGraph*/, Vertex* pVertex)
{
    if(pVertex->getColor() == GC_WHITE)
    {
        EdgePtrVec edges;
        ...
        if(edges.size() == 1)
        {
            std::string scaffoldId = "scaffold_";
            std::string scaffoldSeq = "";
            EdgePtrVec newEdges;
            
            Vertex* nowVertex = pVertex;
            Vertex* lastVertex = pVertex;
            newEdges = nowVertex->getEdges();
            size_t nowForward = 1;
            // size_t lastForward = 1;
            while(1)
            {              
                nowVertex->setColor(GC_BLACK);
                scaffoldId.append(nowVertex->getID()); 
                if(nowForward == 1)
                    scaffoldSeq.append(nowVertex->getStr());
                else
                    scaffoldSeq.append(reverseComplement(nowVertex->getStr()));
                if(newEdges.size() != 1)
                    break;               
                
                lastVertex = nowVertex;
                // lastForward = nowForward;
                nowVertex = newEdges[0]->getEnd(); 
                //--------------------------------------------
                if(newEdges[0]->getComp() == EC_SAME)
                    nowForward = nowForward * 1;
                else
                    nowForward = nowForward * -1;   
                //-------------------------------------------- 
                // if(nowForward == 1)
                ...
                size_t c1_align_pos = 0;
                size_t c2_align_pos = 0;    
                size_t c1_len = lastVertex->getSeqLen();
                size_t c2_len = nowVertex->getSeqLen();
                size_t c1_pos = 0;
                size_t c2_pos = 0;
                ...
                if(newEdges[0]->getComp() == EC_SAME)
                {                    
                    if(newEdges[0]->getDir() == ED_SENSE)
                    {
                    std::cout << "SAME_SENSE" << std::endl;
                    c1_align_pos = newEdges[0]->getC1PosMedian();
                    c2_align_pos = newEdges[0]->getC2PosMedian();
                    c1_pos = c1_len - c1_align_pos;
                    c2_pos = c2_align_pos;
                    }
                    else
                    {
                    std::cout << "SAME_ANTISENSE" << std::endl;
                    c1_align_pos = newEdges[0]->getC2PosMedian();
                    c2_align_pos = newEdges[0]->getC1PosMedian();
                    c1_pos = c1_align_pos;
                    c2_pos = c2_len - c2_align_pos;
                    }
                }
                else
                {
                    if(newEdges[0]->getDir() == ED_SENSE)
                    {
                    std::cout << "REVERSE_SENSE" << std::endl;
                    c1_align_pos = newEdges[0]->getC1PosMedian();
                    c2_align_pos = newEdges[0]->getC2PosMedian();
                    }
                    else
                    {
                    std::cout << "REVERSE_ANTISENSE" << std::endl;
                    c1_align_pos = newEdges[0]->getC2PosMedian();
                    c2_align_pos = newEdges[0]->getC1PosMedian();
                    }
                    c1_pos = c1_align_pos;
                    c2_pos = c2_align_pos;
                    if(c1_align_pos > (c1_len - c1_align_pos))
                        c1_pos = c1_len - c1_align_pos;                    
                    if(c2_align_pos > (c2_len - c2_align_pos))
                        c2_pos = c2_len - c2_align_pos;    
                    
                    // if(c1_align_pos + c2_align_pos <= m_insertSize)
                    // {
                ...
                std::cout << lastVertex->getID() << "to" << nowVertex->getID() << std::endl;
                std::cout << lastVertex->getID() << " len: " << c1_len;
                std::cout << ", pos: " << c1_align_pos/*newEdges[0]->getC1PosMedian()*/ << std::endl;
                if(c1_align_pos > c1_len)
                    std::cout << "-------------------------------------------------------------" << std::endl;
                std::cout << nowVertex->getID() << " len: " << c2_len;
                std::cout << ", pos: " << c2_align_pos/*newEdges[0]->getC2PosMedian()*/ << std::endl;
                if(c2_align_pos > c2_len)
                    std::cout << "-------------------------------------------------------------" << std::endl;
                size_t N_count = m_insertSize - c1_pos - c2_pos;  
                std::cout << "N_count " << N_count << " = " << m_insertSize << " - " << c1_pos << " - " << c2_pos << std::endl;        
                // if(N_count <= 0)
                // std::cout << "**********************************************************" << std::endl;
                ...
                {
                    scaffoldSeq.append("N");
                }
                

                
                newEdges = nowVertex->getEdgesWithout(lastVertex);
            }
            SeqItem scaffold_seq = {scaffoldId, scaffoldSeq};
            scaffoldInfo.push_back(scaffold_seq);
            // std::cout << "newVertex = " << scaffoldId << std::endl;
        }
    }
    return false;
}
void SGScaffoldVisitor::postvisit(StringGraph* pGraph)
{
    // std::cout << "Remove " << pGraph->sweepEdges(GC_BLACK) << " edges." << std::endl;
    std::cout << "Remove " << pGraph->sweepVertices(GC_BLACK) << " vertices." << std::endl;
    for (size_t i = 0; i < scaffoldInfo.size(); i++)
    {
        Vertex* newVertex = new(pGraph->getVertexAllocator()) Vertex(scaffoldInfo[i].id, scaffoldInfo[i].seq.toString());            
        newVertex->setScaSeq(scaffoldInfo[i].seq.toString());
        newVertex->setScaffold(true);
        pGraph->addVertex(newVertex);
    }   
    scaffoldInfo.clear();    
}

void SGRemoveByNpairVisitor::previsit(StringGraph* pGraph)
{
    pGraph->setColors(GC_WHITE);
}
bool SGRemoveByNpairVisitor::visit(StringGraph* /*pGraph*/, Vertex* pVertex)
{
    ...
    {       
        ...
        {                     
            size_t npair;
            npair = edges[dir][i]->getNpair();
            if(npair < m_npairThreshold)
            {
...
void SGRemoveByNpairVisitor::postvisit(StringGraph* pGraph)
{
    std::cout << "Remove " << pGraph->sweepEdges(GC_BLACK) << " edges." << std::endl;
}
void SGRemoveByPosVisitor::previsit(StringGraph* pGraph)
{
    pGraph->setColors(GC_WHITE);
}
bool SGRemoveByPosVisitor::visit(StringGraph* /*pGraph*/, Vertex* pVertex)
{
    EdgePtrVec edges[ED_COUNT];
    // std::cout << "*********************************" << std::endl; 
	edges[ED_SENSE] = pVertex->getEdges(ED_SENSE) ;
    ...
    {        
        ...
        {                     
            // std::cout << "pVertex ID = " << pVertex->getID() << std::endl; 
            // std::cout << "pWertex ID = " << edges[idx][i]->getEnd()->getID() << std::endl; 
            Edge * pEdge = edges[idx][i];
            
            // std::cout << "pVertex Length = " << pVertex->getSeqLen(); 
            // std::cout << "\tpWertex Length = " << edges[idx][i]->getEnd()->getSeqLen() << std::endl; 

            // if(pEdge->getComp() == EC_SAME)
                // std::cout << "EC_SAME, " ; 
            // else
                // std::cout << "EC_REVERSE, "; 
                
            // if(pEdge->getDir() == ED_SENSE)
                // std::cout << "ED_SENSE" << std::endl; 
            // else
                // std::cout << "ED_ANTISENSE" << std::endl;  
            
            size_t c1_align_pos1;
            size_t c1_align_pos2;
            size_t c2_align_pos1;
            size_t c2_align_pos2;
         
            c1_align_pos1 = pEdge->getC1PosMedian();
            c1_align_pos2 = pVertex->getSeqLen() - pEdge->getC1PosMedian();
            c2_align_pos1 = pEdge->getC2PosMedian();
            c2_align_pos2 = pEdge->getEnd()->getSeqLen() - pEdge->getC2PosMedian();
            
            // std::cout << "c1_align_pos1 = " << c1_align_pos1 << ", c1_align_pos2 = " << c1_align_pos2 << std::endl; 
            // std::cout << "c2_align_pos1 = " << c2_align_pos1 << ", c2_align_pos2 = " << c2_align_pos2 << std::endl;
            
            size_t N_Length;
            if(pEdge->getComp() == EC_SAME)
            {
                if(pEdge->getDir() == ED_SENSE)
                    N_Length = c1_align_pos1 + c2_align_pos2;
                else
                    N_Length = c1_align_pos2 + c2_align_pos1;
            }
            else
            {
                if(pEdge->getDir() == ED_SENSE)
                    N_Length = c1_align_pos1 + c2_align_pos1;
                else
                    N_Length = c1_align_pos2 + c2_align_pos2;    
            }                             
            if(N_Length > m_insertSize)
            {
            ...
            {   
                ...
                pEdge->setNLength(N_Length); 
                pEdge->getTwin()->setNLength(N_Length); 
            }    
        }        
    }
    return false;
}
void SGRemoveByPosVisitor::postvisit(StringGraph* pGraph)
{
    std::cout << "Remove " << pGraph->sweepEdges(GC_BLACK) << " edges." << std::endl;
}
void SGCalcInsertSizeVisitor::previsit(StringGraph* /*pGraph*/)
{
}
bool SGCalcInsertSizeVisitor::visit(StringGraph* /*pGraph*/, Vertex* pVertex)
{
    size_t firstStartPos = 0;
    size_t firstEndPos = 0;
    size_t secondStartPos = 0;
    size_t secondEndPos = 0;
    size_t insertDistance = 0;
    size_t npair = 0;
    
    size_t firstId = 0;
    
    std::string ID1 = pVertex->getID();
    std::stringstream ss(ID1);
    std::string sub_str;
    size_t count = 0;
    while(getline(ss,sub_str,'_'))
    {
        if(count == 0)
            firstId = atoi(sub_str.c_str());
        if(count == 1)
            firstStartPos = atoi(sub_str.c_str());
        if(count == 2)
            firstEndPos = atoi(sub_str.c_str());
        count += 1;
    }
    ...
    {        
        ...
        {
            MAPPINGINFO temp;
            temp.id1 = pVertex->getID();            
            temp.id2 = "NULL";
            temp.EC_COMP = "NULL";
            temp.insertDistance = 0;
            temp.npair = 0;
            temp.c1Pos = 0;
            temp.c1_len = 0;            
            temp.c2Pos = 0;
            ...
            // for sort
            temp.id1_s = firstStartPos;
            temp.id = firstId;              
            
            mappingInfo.push_back(temp);
        }
        for (size_t i=0; i < edges[idx].size(); i++)
        {                     
            Edge * pEdge = edges[idx][i];
            npair = pEdge->getNpair();
            Vertex* pW = pEdge->getEnd();
            
            std::string ID2 = pW->getID();
            ...
            {
                if(count == 1)
                    secondStartPos = atoi(sub_str.c_str());
                if(count == 2)
                    secondEndPos = atoi(sub_str.c_str());
                count += 1;
            }      
            
            if(firstStartPos <= secondStartPos)
            {
                if(secondStartPos <= firstEndPos)
                {
                    insertDistance = firstEndPos - secondStartPos;
                }
                else
                    insertDistance = secondStartPos - firstEndPos;
            }
            else
            {
                if(firstStartPos <= secondEndPos)
                {
                    insertDistance = secondEndPos - firstStartPos;
                }
                else
                    insertDistance = firstStartPos - secondEndPos;
            }          
            MAPPINGINFO temp;            
            temp.id1 = pVertex->getID(); 
            temp.id2 = pW->getID(); 
            if(pEdge->getComp() == EC_SAME)
                ...
                temp.EC_COMP = "REVERSE";   
            temp.insertDistance = insertDistance;
            temp.npair = npair;             
            temp.c1Pos = pEdge->getC1PosMedian();
            temp.c1_len = pVertex->getSeqLen();            
            temp.c2Pos = pEdge->getC2PosMedian();
            temp.c2_len = pW->getSeqLen();          
            ...
            {
                struct stat buf;
                std::string dirname = "matepairInfo/";
                
                if(stat(dirname.c_str(), &buf) != 0)   
                {
                    mkdir(dirname.c_str(), S_IRWXU);
                    // std::cout << "Create a dir:" << dirname << std::endl;
                }
                    
                FILE *fp;
                std::vector<SeqItem> r1_reads = pEdge->getR1Reads();                    
                std::vector<SeqItem> r2_reads = pEdge->getR2Reads();
                
                std::string filename;
                std::stringstream ssf;
                ssf << dirname << "matepair_" << pVertex->getID() << "_" << 
                    pW->getID() << "_npair_" << npair << ".fa";
                ssf >> filename;
                
                fp = fopen(filename.c_str(),"w");
                for(size_t k = 0; k < r1_reads.size(); k++)
                {
                    fprintf(fp,">%s\n", r1_reads[k].id.c_str());
                    fprintf(fp,"%s\n", r1_reads[k].seq.toString().c_str());
                    fprintf(fp,">%s\n", r2_reads[k].id.c_str());
                    fprintf(fp,"%s\n", r2_reads[k].seq.toString().c_str());
                }
                fclose(fp);
            }
        }        
...
void SGCalcInsertSizeVisitor::postvisit(StringGraph* /*pGraph*/)
{
    FILE *fp;
    sort(mappingInfo.begin(), mappingInfo.end(), compare2); 
    fp = fopen(m_filename.c_str(), "w");
    for(size_t i = 0; i < mappingInfo.size(); i++)
    {
        fprintf(fp,"%s\t", mappingInfo[i].id1.c_str());
        fprintf(fp,"%s\t", mappingInfo[i].id2.c_str());
        fprintf(fp,"%s\t", mappingInfo[i].EC_COMP.c_str());
        fprintf(fp,"%-d\t", (int)mappingInfo[i].insertDistance);
        fprintf(fp,"%-d\t", (int)mappingInfo[i].npair);
        fprintf(fp,"%-d\t", (int)mappingInfo[i].c1Pos);
        fprintf(fp,"%-d\t", (int)mappingInfo[i].c1_len);
        fprintf(fp,"%-d\t", (int)mappingInfo[i].c2Pos);
        fprintf(fp,"%-d\n", (int)mappingInfo[i].c2_len);
    }
    fclose(fp);
    mappingInfo.clear();
}
bool compare2(const MAPPINGINFO & s1, const MAPPINGINFO & s2)
{
    if(s1.id1_s != s2.id1_s) 
    {
        return s1.id1_s < s2.id1_s;
    }
    else  
    {
        if(s1.id != s2.id)
...
size_t SGRetentionMaxNpairVisitor::getDir(Edge* edge)
{
    size_t dir;
    if(edge->getComp() == EC_SAME)
    {
...
void SGRetentionMaxNpairVisitor::previsit(StringGraph* pGraph)
{
    pGraph->setColors(GC_WHITE);
}
bool SGRetentionMaxNpairVisitor::visit(StringGraph* /*pGraph*/, Vertex* pVertex)
{
    EdgePtrVec Vedges;
	Vedges = pVertex->getEdges();
    // std::cout << "************** Vertex: " << pVertex->getID() << " has " << Vedges.size() << " edges." << std::endl;
    
    // if(Vedges.size() >= 2)
    // {
        // idx Npair dir
        size_t FirstEdge[3] = {0};
        size_t SecondEdge[3] = {0};
        bool second_exist = false;
        for(size_t idx = 0; idx < Vedges.size(); idx++)	
        {       
            // size_t lastDir = getDir(Vedges[idx]);
            // std::cout << "W:" << Vedges[idx]->getEnd()->getID() <<" dir: " << getDir(Vedges[idx]) << std::endl;
            size_t EdgeNpair = Vedges[idx]->getNpair();
            Vertex* lastVertex = pVertex;
            Vertex* NewVertex = Vedges[idx]->getEnd();
            EdgePtrVec NewEdges = NewVertex->getEdgesWithout(lastVertex);
            bool pass = false;
            while(1)
            {
                if(NewEdges.size() != 1)
                ...
                // if(NewVertex == lastVertex)
                    // continue;
                // std::cout << "Y:" << NewVertex->getID() <<" dir: " << getDir(NewEdges[0]) << std::endl;    
                // size_t newDir = getDir(NewEdges[0]);
                // if( lastDir != 2 && newDir != 2)  
                // {
                    // if(newDir != lastDir)
                        // break;
                // }
                
                EdgeNpair += NewEdges[0]->getNpair();
                // lastDir = newDir;
                lastVertex = NewVertex;
                NewVertex = NewEdges[0]->getEnd();
                if(NewVertex == pVertex)
                {
                    if(Vedges[idx]->getNpair() < NewEdges[0]->getNpair())
                    {
                        std::cout << Vedges[idx]->getStart()->getID() << " to " << Vedges[idx]->getEnd()->getID() << std::endl;
                        Vedges[idx]->setColor(GC_BLACK);
                        Vedges[idx]->getTwin()->setColor(GC_BLACK);
                        pass = true;
                    }
                }
                NewEdges = NewVertex->getEdgesWithout(lastVertex);
                // }
            }
            
            // std::cout << "EdgeNpair: " << EdgeNpair << std::endl;
            if(pass == true)
                continue;
            
            if(EdgeNpair > FirstEdge[1])
            {
                // std::cout << "1111111111" <<std::endl;
                size_t tempEdge[3] = {0};
                tempEdge[0] = FirstEdge[0];
                tempEdge[1] = FirstEdge[1];
                tempEdge[2] = FirstEdge[2];  
                FirstEdge[0] = idx;
                FirstEdge[1] = EdgeNpair;                
                FirstEdge[2] = getDir(Vedges[idx]);
                if(tempEdge[1] > SecondEdge[1])
                {
                    // if(FirstEdge[2] != 2 && tempEdge[2] == FirstEdge[2])
                    // {
                        // std::cout << Vedges[tempEdge[0]]->getStart()->getID() << " to " << Vedges[tempEdge[0]]->getEnd()->getID() << std::endl;
                        // Vedges[tempEdge[0]]->setColor(GC_BLACK);
                        // Vedges[tempEdge[0]]->getTwin()->setColor(GC_BLACK);
                    // }
                    // else
                    // {          
                        if(second_exist == true)
                        {
                        // std::cout << Vedges[SecondEdge[0]]->getStart()->getID() << " to " << Vedges[SecondEdge[0]]->getEnd()->getID() << std::endl;
                        Vedges[SecondEdge[0]]->setColor(GC_BLACK);
                        Vedges[SecondEdge[0]]->getTwin()->setColor(GC_BLACK);
                        }
                        SecondEdge[0] = tempEdge[0];
                        SecondEdge[1] = tempEdge[1];
                        SecondEdge[2] = tempEdge[2];
                        second_exist = true;
                    // }
                }
            }
            else if(EdgeNpair > SecondEdge[1])
            {
                // std::cout << "222222222222" <<std::endl;
                size_t tempEdge[3] = {0};
                tempEdge[0] = idx;
                tempEdge[1] = EdgeNpair;                
                tempEdge[2] = getDir(Vedges[idx]);
                // if(FirstEdge[2] != 2 && tempEdge[2] == FirstEdge[2])
                // {
                    // std::cout << Vedges[tempEdge[0]]->getStart()->getID() << " to " << Vedges[tempEdge[0]]->getEnd()->getID() << std::endl;
                    // Vedges[tempEdge[0]]->setColor(GC_BLACK);
            ...
            // std::cout << "First" << Vedges[FirstEdge[1]]->getStart()->getID()
            // << " to " << Vedges[FirstEdge[1]]->getEnd()->getID()
            // << " Npair: " << Vedges[FirstEdge[1]]->getNpair() << std::endl;
            // std::cout << "Second" << Vedges[SecondEdge[1]]->getStart()->getID()
            // << " to " << Vedges[SecondEdge[1]]->getEnd()->getID()
            // << " Npair: " << Vedges[SecondEdge[1]]->getNpair() << std::endl;
    // }
    return false;
}
void SGRetentionMaxNpairVisitor::postvisit(StringGraph* pGraph)
{
    std::cout << "Remove " << pGraph->sweepEdges(GC_BLACK) << " edges." << std::endl;
}

void SGRemoveCircleEdgeVisitor::previsit(StringGraph* pGraph)
{
	pGraph->setColors(GC_WHITE);
}
bool SGRemoveCircleEdgeVisitor::visit(StringGraph* , Vertex* pVertex)
{
    EdgePtrVec edges[ED_COUNT];
	edges[ED_SENSE] = pVertex->getEdges(ED_SENSE);
    edges[ED_ANTISENSE] = pVertex->getEdges(ED_ANTISENSE);
    // std::cout << "pV ID: " << pVertex->getID() << std::endl;
    for (size_t idx = 0; idx < 2; idx++)
    {
        // std::cout << "edge size: " << edges[idx].size() << std::endl;
        EdgeDir dir = EDGE_DIRECTIONS[idx];
        for (size_t i = 0; i < edges[idx].size(); i++)
        {                         
            Edge * pEdgeV = edges[idx][i];            
            Vertex* pW = pEdgeV->getEnd();
            EdgePtrVec w_edges = pVertex->getEdgesWith(dir, pW);
            // std::cout << "pW ID: " << pW->getID() << std::endl;
            // std::cout << "w_edges size: " << w_edges.size() << std::endl;
            
            Edge * pMaxEdge = w_edges[0];  
            for (size_t j = 1; j < w_edges.size(); j++)
            {
                if(w_edges[j]->getNpair() > pMaxEdge->getNpair())
                {
                        pMaxEdge->setColor(GC_BLACK);
                        pMaxEdge->getTwin()->setColor(GC_BLACK);
                        pMaxEdge = w_edges[j];                        
                }
                else if (w_edges[j]->getNpair() == pMaxEdge->getNpair())
                {
                    w_edges[j]->setColor(GC_BLACK);
                    w_edges[j]->getTwin()->setColor(GC_BLACK);
                    pMaxEdge->setColor(GC_BLACK);
                    pMaxEdge->getTwin()->setColor(GC_BLACK);
                }
                else
...
void SGRemoveCircleEdgeVisitor::postvisit(StringGraph* pGraph)
{
	std::cout << "Remove " << pGraph->sweepEdges(GC_BLACK) << " edges." << std::endl;
}

void SGRemoveSplitEdgeVisitor::previsit(StringGraph* pGraph)
{
	pGraph->setColors(GC_WHITE);
}
bool SGRemoveSplitEdgeVisitor::visit(StringGraph* , Vertex* pVertex)
{
    EdgePtrVec edges[ED_COUNT];
	edges[ED_SENSE] = pVertex->getEdges(ED_SENSE);
    edges[ED_ANTISENSE] = pVertex->getEdges(ED_ANTISENSE);
    ...
    for (size_t idx = 0; idx < 2; idx++)
    {
        ...
        if(edges[idx].size() > 1)
        {
            Edge * pMaxEdge = edges[idx][0];  
            for (size_t i = 1; i < edges[idx].size(); i++)
            {                         
                if(edges[idx][i]->getNpair() > pMaxEdge->getNpair())
                {
                    pMaxEdge->setColor(GC_BLACK);
                    pMaxEdge->getTwin()->setColor(GC_BLACK);
                    pMaxEdge = edges[idx][i];                        
                }
                else if (edges[idx][i]->getNpair() == pMaxEdge->getNpair())
                {
                    edges[idx][i]->setColor(GC_BLACK);
                    edges[idx][i]->getTwin()->setColor(GC_BLACK); 
                    pMaxEdge->setColor(GC_BLACK);
                    pMaxEdge->getTwin()->setColor(GC_BLACK);
                }
                else
...
// Remove all the marked edges
void SGRemoveSplitEdgeVisitor::postvisit(StringGraph* pGraph)
{
	std::cout << "Remove " << pGraph->sweepEdges(GC_BLACK) << " edges." << std::endl;
}
void SGScaTransitiveReductionVisitor::previsit(StringGraph* pGraph)
{
	pGraph->setColors(GC_WHITE);
}
bool SGScaTransitiveReductionVisitor::visit(StringGraph* /*pGraph*/, Vertex* pVertex)
{
    size_t trans_count = 0;
    // std::cout << "pVertex = " << pVertex->getID() << std::endl;
    for(size_t idx = 0; idx < 2; idx++)
    {
        EdgeDir dir = EDGE_DIRECTIONS[idx];
        ...
        {            
            ...
            // std::cout << "pW = " << edges[i]->getEnd()->getID() << " color = GC_GRAY" << std::endl;
        }
        // Edge* ed = NULL;
        for(size_t i = 0; i < edges.size(); ++i)
        {
            ...
            if(pWVert->getColor() == GC_GRAY || pWVert->getColor() == GC_BLACK)
            {                
                // EdgePtrVec w_edges = pWVert->getEdgesBoth(transDir);
                EdgePtrVec w_edges = pWVert->getEdgesBoth(dir);
                for(size_t j = 0; j < w_edges.size(); ++j)
                {
                    Edge* pWXEdge = w_edges[j];
                    if(pWXEdge->getEnd()->getColor() == GC_GRAY && pWXEdge->getComp() != EC_REVERSE)//&& pWXEdge->getEnd() != pVertex)
                    {
                    // if(pWXEdge->getComp() == EC_SAME)
                    // std::cout << "pWVert = " << pWVert->getID() << ", S pWWW = " << pWXEdge->getEnd()->getID() << std::endl;
                    // else
                    // std::cout << "pWVert = " << pWVert->getID() << ", R pWWW = " << pWXEdge->getEnd()->getID() << std::endl;
                        pWXEdge->getEnd()->setColor(GC_BLACK);
                        // if(pVWEdge->getNpair() >= pWXEdge->getNpair())
                            // ed = pWXEdge;
                        // else
                            // ed = pVWEdge;
                    }
        ...
        for(size_t i = 0; i < edges.size(); ++i)
        {
            if(edges[i]->getEnd()->getColor() == GC_BLACK)
            {
                ...
                {
                    // if(ed->getNpair() > edges[i]->getNpair())
                    // {
                    // std::cout << "edNpair " << ed->getNpair() << " > " << edges[i]->getNpair() << std::endl;
                    // std::cout << "the edge " << edges[i]->getStart()->getID() << " to " << edges[i]->getEnd()->getID() << std::endl;
                    ...
                    // else
                    // {
                    // ed->setColor(GC_BLACK);
                    // ed->getTwin()->setColor(GC_BLACK);
                    // marked_edges += 2;
                    // trans_count ++;                        
                    // }
                }
...
void SGScaTransitiveReductionVisitor::postvisit(StringGraph* pGraph)
{
	std::cout << "Remove " << pGraph->sweepEdges(GC_BLACK) << " transitive edges." << std::endl;
