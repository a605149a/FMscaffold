...
Edge* SGAlgorithms::createScaffoldEdges(StringGraph* pGraph, std::string StartID, std::string EndID,
                                        std::string EC_comp, std::string ED_dir, size_t npair, size_t C1Pos, size_t C2Pos, GraphColor c)
{
    // Initialize data and perform checks
    Vertex* pVerts[2];
    pVerts[0] = pGraph->getVertex(StartID);
    pVerts[0]->setColor(c);   
    pVerts[1] = pGraph->getVertex(EndID);
    pVerts[1]->setColor(c);
    ...
    c2Pos[0] = C2Pos;
    c2Pos[1] = C1Pos;
    if(EC_comp.compare("EC_SAME") == 0)
    {
        if(ED_dir.compare("ED_SENSE") == 0)
        {
            dir[0] = ED_SENSE;
            dir[1] = ED_ANTISENSE;
            // c1Pos[0] = C1Pos;
    ...
    for(size_t idx = 0; idx < 2; ++idx)
    {
            ...
            else
            {
                ...
                pEdges[idx] = new Edge(pVerts[1 - idx], dir[idx], ec_comp, npair, c1Pos[idx], c2Pos[idx], c);
    }
