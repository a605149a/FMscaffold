...
// make a scaffold fasta
struct SGFastaVisitor2
{
    // constructor
    SGFastaVisitor2(std::string filename) : m_fileHandle(filename.c_str()) {}
    ~SGFastaVisitor2() { m_fileHandle.close(); }
...
class MAPPINGINFO
{
    public:
        ...
        std::string id2;
        std::string EC_COMP;
        size_t insertDistance;
        size_t npair;
        
        size_t insertSize;
        size_t edge_s;
};
struct SGScaffoldVisitor
{
    SGScaffoldVisitor(size_t insertSize) : m_insertSize(insertSize){}
    ~SGScaffoldVisitor(){}
    ...
    size_t m_insertSize;    
    std::vector<SeqItem> scaffoldInfo;
};
struct SGRemoveByNpairVisitor
{
    SGRemoveByNpairVisitor(size_t npairThreshold): m_npairThreshold(npairThreshold){}
    ~SGRemoveByNpairVisitor(){}

    void previsit(StringGraph* pGraph);
    bool visit(StringGraph* pGraph, Vertex* pVertex);
    void postvisit(StringGraph* pGraph);
    
    size_t m_npairThreshold;
};
struct SGRemoveByPosVisitor
{
    SGRemoveByPosVisitor(size_t insertSize) : m_insertSize(insertSize){}
    ~SGRemoveByPosVisitor(){}
    ...
    size_t m_insertSize;    
    std::vector<INSERTSIZEINFO> insertInfo;
    std::vector<MAPPINGINFO> mappingInfo;
};
bool compare2(const MAPPINGINFO & s1, const MAPPINGINFO & s2);
// Calculate inner distance of Every Edge
struct SGCalcInsertSizeVisitor
{
    // constructor
    SGCalcInsertSizeVisitor(size_t insertSize, std::string filename, bool skipPrint) : 
                        m_insertSize(insertSize), m_filename(filename), m_skipPrint(skipPrint) {}
    ~SGCalcInsertSizeVisitor(){}
...
// Retention Max Npair Edge
struct SGRetentionMaxNpairVisitor
{
    // constructor
    SGRetentionMaxNpairVisitor(){}
    ~SGRetentionMaxNpairVisitor(){}
    // functions
    void previsit(StringGraph* pGraph);
    bool visit(StringGraph* pGraph, Vertex* pVertex);
    void postvisit(StringGraph* pGraph);
    size_t getDir(Edge* edge);
};

// 
struct SGRemoveCircleEdgeVisitor
{
    // constructor
    SGRemoveCircleEdgeVisitor(){}
    ~SGRemoveCircleEdgeVisitor(){}
...
// 
struct SGRemoveSplitEdgeVisitor
{
    // constructor
    SGRemoveSplitEdgeVisitor(){}
    ~SGRemoveSplitEdgeVisitor(){}
...
struct SGScaTransitiveReductionVisitor
{
    SGScaTransitiveReductionVisitor() {}
    ~SGScaTransitiveReductionVisitor() {}
    void previsit(StringGraph* pGraph);
    bool visit(StringGraph* pGraph, Vertex* pVertex);
