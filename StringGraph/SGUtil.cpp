...
StringGraph* SGUtil::loadScaffoldASQG(const std::string& filename, GraphColor c)
{
	// Initialize graph
			...
			case ASQG::RT_EDGE:
				{
					...
					ASQG::ScaEdgeRecord scaEdgeRecord(recordLine);
                    std::string StartID = scaEdgeRecord.getStartID();
                    std::string EndID = scaEdgeRecord.getEndID();
                    // std::cout << StartID << "->" << EndID << std::endl;
                    std::string EC_comp = scaEdgeRecord.getECcomp();
                    std::string ED_dir = scaEdgeRecord.getEDdir();
					size_t npair = scaEdgeRecord.getNpair();
                    size_t C1Pos = scaEdgeRecord.getC1Pos();
                    size_t C2Pos = scaEdgeRecord.getC2Pos();
                    
					// Add the edge to the graph
                    SGAlgorithms::createScaffoldEdges(pGraph, StartID, EndID, EC_comp, ED_dir, npair, C1Pos, C2Pos, c);
					break;
				}
		}
		++line;
